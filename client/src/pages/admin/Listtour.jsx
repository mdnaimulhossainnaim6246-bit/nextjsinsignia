...
// src/pages/admin/Listtour.jsx

import React, { useEffect, useState } from 'react';
import TourTable from '../../components/addmin/TourTable';
...
const Listtour = () => {
  ...
  const fetchDashboard = async () => {
    try {
      ...
      if (data.success) {
        setTourList(data.tour);

        const uploaded = data.tour.filter((item) => item.isPublished).length;
        const drafts = data.tour.filter((item) => !item.isPublished).length;
          ...
          <table className="table">
            <thead className="table-thead">
              <tr className="data-table-tr data-table-tr-thead">
                ...
                <th scope="col" className="table-th-title">CITY</th>
